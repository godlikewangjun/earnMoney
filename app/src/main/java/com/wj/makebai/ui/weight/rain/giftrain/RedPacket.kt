 ...
 */
class RedPacket {
    var index = -1 //红包在红包流中的位置（未包含彩带）。
    private var mX = 0
    private var mY=0 //飘落物此刻的坐标。 = 0
    var imageRes=0 //飘落物的图片资源。 = 0
    private var mType = TYPE_PACKET //飘落物体的类型。
    private var mTypeIndex=0 //飘落物当前类型的索引（每帧自增）。 = 0
    ...
    companion object {
        const val TYPE_PACKET = 1 //只有这个类型可以点击
        const val TYPE_PACKET_OPEN = 3 //开启后
        const val TYPE_BOOM = 5 //爆炸
        const val TYPE_GIFT = 7 //礼物
        const val TYPE_RIBBON = 12 //彩带
    }
}