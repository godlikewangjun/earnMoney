...
import com.wj.makebai.ui.adapter.SearchTagsAdapter
import com.wj.makebai.ui.adapter.TagsAdapter
...
import kotlinx.android.synthetic.main.activity_search_novel_layout.back
import kotlinx.android.synthetic.main.activity_search_novel_layout.clear_text
import kotlinx.android.synthetic.main.activity_search_novel_layout.pushRecyclerView
import kotlinx.android.synthetic.main.activity_search_novel_layout.recyclerView
import kotlinx.android.synthetic.main.activity_search_novel_layout.refresh_layout
import kotlinx.android.synthetic.main.activity_search_novel_layout.searchView
import kotlinx.android.synthetic.main.activity_search_novel_layout.search_history
import kotlinx.android.synthetic.main.activity_search_novel_layout.view1
 ...
 * @date 2019/9/20
 */
class SearchNovelActivity : MakeActivity(), View.OnClickListener {
    private var moviesAdapter: NovelListAdapter? = null
    private var isLoad = false
    private var search = ""
    private var page = 1
    override fun bindLayout(): Int {
        return R.layout.activity_search_novel_layout
    }
    ...
    override fun initData() {
        ...
        pushRecyclerView.adapter = TagsAdapter().apply {
            list = arrayListOf("斗破苍穹", "鬼吹灯", "盗墓笔记", "神墓", "舞动乾坤")
        ...
        clear_text.setOnClickListener {
            activity.database.use {
                lifecycleScope.launch(Dispatchers.IO) {
                    AppDatabase.db.searchHistoryDao().removeAll( TypesEnum.NOVEL.ordinal)
                }
        ...
        searchView.setOnSearchListener={
            ...
            lifecycleScope.launch {
                AppDatabase.db.searchHistoryDao().delete(search, TypesEnum.NOVEL.ordinal)
                AppDatabase.db.searchHistoryDao().insert(SearchHistoryMode(0,search, TypesEnum.NOVEL.ordinal))
            }
    ...
    private fun loadHistory(search_text: MySearchView) {
        AppDatabase.db.searchHistoryDao().getAll(TypesEnum.NOVEL.ordinal).observe(this){
            if (it.isEmpty()) {
    ...
    override fun onBackPressed() {
        if (recyclerView.visibility == View.VISIBLE &&
            ...
            (recyclerView.adapter as NovelListAdapter).list.isNotEmpty()) {
            (recyclerView.adapter as NovelListAdapter).apply {
                list.clear()
                notifyDataSetChanged()
            }
